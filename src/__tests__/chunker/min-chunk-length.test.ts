...
describe('SemanticChunker minChunkLength', () => {
  it('should use default minChunkLength of 200 when not specified', async () => {
    const chunker = new SemanticChunker()
    const embedder = createDissimilarEmbedder()

    // Create text with sentences that produce chunks shorter than 200 chars
    const shortSentence = 'Short sentence here.'
    // Each sentence is ~20 chars, well under 200
    const text = Array(5).fill(shortSentence).join(' ')

    const chunks = await chunker.chunkText(text, embedder)

    // All individual chunks are under 200 chars, so they should be filtered out
