...
describe('SemanticChunker minChunkLength', () => {
  ...
  // ---- skipShortFiles tests ----

  it('should keep single short chunk when skipShortFiles is false (default)', async () => {
    const chunker = new SemanticChunker({ minChunkLength: 5000 })
    const embedder = createMockEmbedder()
    ...
    // Default: skipShortFiles is false, so single short chunk is kept
    expect(chunks).toHaveLength(1)
    expect(chunks[0]!.text).toContain('Short file content')
  })

  it('should reject single short chunk when skipShortFiles is true', async () => {
    const chunker = new SemanticChunker({ minChunkLength: 5000, skipShortFiles: true })
    const embedder = createMockEmbedder()

    const text = 'Short file content.'
    const chunks = await chunker.chunkText(text, embedder)

    // Single chunk below minChunkLength is rejected
    expect(chunks).toHaveLength(0)
  })

  it('should keep all chunks in multi-chunk output even with skipShortFiles enabled', async () => {
    const chunker = new SemanticChunker({ minChunkLength: 50, skipShortFiles: true })
    const embedder = createDissimilarEmbedder()

    // Long enough text with dissimilar sentences to produce multiple chunks
    const text = [
      'The authentication module handles OAuth 2.0 token validation and verifies JWT signatures against public keys.',
      'Database sharding distributes data across multiple PostgreSQL instances for horizontal scalability and high availability.',
      'The caching layer uses Redis with a least-recently-used eviction policy and configurable time-to-live settings.',
    ].join(' ')

    const chunks = await chunker.chunkText(text, embedder)

    // Multi-chunk output: skipShortFiles does not apply
    expect(chunks.length).toBeGreaterThan(1)
    const allText = chunks.map((c) => c.text).join(' ')
    expect(allText).toContain('authentication')
    expect(allText).toContain('Database sharding')
    expect(allText).toContain('caching layer')
  })
})
